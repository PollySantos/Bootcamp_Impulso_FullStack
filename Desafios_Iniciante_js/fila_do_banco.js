...
let nClientes, senhasOrdenadas, contadorSemTroca;
let senhas = [];

for (let n = 0, nCasos = parseInt(gets()); n < nCasos; n++) {
  nClientes = parseInt(gets());
  senhas[n] = gets().split(' ');

  senhasOrdenadas = Array.from(senhas[n]).sort((a, b) => b - a);
  contadorSemTroca = 0;
  
  for (let i = 0; i < senhas[n].length; i++) { senhas[n][i] === senhasOrdenadas[i] && (contadorSemTroca++); }
